...
students_resource = StudentsResource("./resources/old-students.json")
...
async def say_hello(name: str):
    return {"message": f"Awesome cloud developer yz4359 says hello {name}"}


@app.get("/hello_text/{name}")
async def say_hello_text(name: str):
    the_message = f"Awesome cloud developer yz4359 says Hello {name}"
